...
class mymodel:
    ...
    def to_excel(self, inst: Instance):
        filename = 'results/' + inst.name + "_res.xlsx"
...
def build_model(inst : Instance):
    ...
    y = {(i,j,s) :mm.addVar(vtype = GRB.BINARY, name = 'y_{}_{}_{}'.format(i,j,s)) for i in range(1, inst.n+1) for j in inst.Vs for s in inst.S if j!=i }
    ...
    mm.addConstrs((R[j,s] - Q[j,s] + inst.C >= 0 for s in inst.S for j in inst.Vs), name = 'ct12') #range(inst.n + 1, inst.n + inst.m + 1)
    ...
    mm.addConstrs((lambd[j,s] - ((1/inst.Qmax)*(Q[i,s] - inst.C)) + inst.M * (1 - x[i,j]) >= 0 for s in inst.S for i in inst.Vp for j in inst.V1 if j != i), name = 'ct18')
    mm.addConstrs((lambd[j,s] + ((1/inst.Qmax)*(inst.C - Q[i,s])) - inst.M * (1 - x[i,j]) <= 1 for s in inst.S for i in inst.Vp for j in inst.V1 if j != i), name = 'ct19')

    mm.addConstrs((alpha[j,s] - rho[j,s] + lambd[j,s] >= 0 for s in inst.S for j in inst.V1), name = 'ct20')

    mm.addConstrs(( alpha[j,s] == gb.quicksum(y[j,i,s] for i in inst.Vs if j!=i) - gb.quicksum(y[i,j,s] for i in range(1, inst.n+1) if j!=i)  for s in inst.S for j in range(1, inst.n + 1)), name = 'ctg')
    ...
    print(ZOB4.getValue())
    print(ZOB5.getValue())
    mm.write('results/out_'+inst.name+'.sol')
    return mym

def run_model(inst : Instance, mm : gb.Model):
    mm = mym.m
    mm.Params.TimeLimit = max_runtime
    #mm.optimize()

    #print('Optimizatoin ended with status (2=optimal, 3=infeasible, 5=inf_or_unbounded, 9=timlimit, 11=interrupted)', mm.Status)
    #if mm.Status in [2,9,11]:
    #    print('object value: ', mm.ObjVal)
    #    mm.write('results/out_'+inst.name+'.sol')
